...
async function main() {
	try {
		...
		try {
			...
			let nb_tickets = process.argv[3];
			if (nb_tickets === undefined) {
				nb_tickets = 5;
			}

			//console.log('\n--> Evaluate Transaction: ReadStatus, function returns rank of clientId');
			console.log('\n--> Evaluate Transaction: CreateAsset, function creates new queue assetId');
			await contract.evaluateTransaction('CreateAsset', assetId, 0, nb_tickets, [], []);
